...
import { AddByUrlPayload, Recommendation, WishlistResponse } from '@/types/wishlist.type';
...
export const getWishlist = async (id: string): Promise<WishlistResponse> => {
  const response = await axios.get<WishlistResponse>(`${process.env.NEXT_PUBLIC_API_URL}/wishlist/${id}`);
  return response.data;
};
...
export const updateBoughtBy = (
  ...
  axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/wishlist/${wishlistId}/items/${itemId}/bought-by`, {
    buyer,
  });
...
export const addByUrl = async ({ url, wishlistId }: AddByUrlPayload) => {
  try {
    const { data } = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/bol/by-url`, {
      url,
      wishlistId,
    });
    return data;
...
export const sendEmail = async ({ to, shareLink }: { to: string; shareLink: string }) => {
  try {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/email/send`, {
      to,
      shareLink,
    });
